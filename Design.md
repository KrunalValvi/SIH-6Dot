...
Each status gets a distinct semantic color — never default to coral for status (coral stays reserved for primary CTA per the Do's/Don'ts above).
...
**`chart-bar`, `chart-line`** — Used for skill-gap heatmaps, placement trend lines, application funnel. Chart canvas background transparent (sits on `{colors.canvas}` or `stat-card`/`feature-card`). Primary data series uses `{colors.primary}` (coral); secondary series use `{colors.accent-teal}`, then `{colors.accent-amber}`; never introduce a 4th chart color outside these three plus `{colors.muted-soft}` for gridlines/axis labels. Tooltip: `{colors.surface-dark}` background, `{colors.on-dark}` text, `{rounded.md}`.

**`chart-heatmap`** — Skill-gap heatmap (Institution panel). Cell intensity scales from `{colors.surface-soft}` (low) to `{colors.primary}` (high) — a single-hue coral ramp, not a red/green diverging scale, to stay within the brand's warm palette.
...
- All panel dashboards (Student, Industry, Academician, Institution, Admin) use `{colors.canvas}` as the base — the app does **not** introduce a separate cool-gray "app mode"; it stays inside the cream/coral/dark-navy trinity already defined.
